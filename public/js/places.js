...
async function displayPlaces(places) {
  ...
  const placesHTML = places
    .map(
      (place, index) => `
    <div class="place-card mb-3" onclick="showPlaceDetails('${
      place.place_id
    }', '${place.name}', '${place.rating || "N/A"}', '${
        place.vicinity || "Address not available"
      }', '${place.types ? place.types[0] : "place"}')">
        ...
        <div class="row g-0">
          ...
          <div class="col-md-8">
            <div class="card-body">
                ...
                ${
                  place.types
                    ? place.types
                        ...
                        .map(
                          (type) =>
                            `<span class="badge bg-secondary me-1">${type.replace(
                              /_/g,
                              " "
                            )}</span>`
                        )
              ...
              <div class="mt-2">
                <button class="btn btn-sm btn-outline-primary me-2" onclick="event.stopPropagation(); openGoogleMaps('${
                  place.name
                }', '${place.vicinity || ""}')">
                  <i class="fas fa-map-marker-alt me-1"></i>Directions
                </button>
                <button class="btn btn-sm btn-outline-info" onclick="event.stopPropagation(); showPlaceDetails('${
                  place.place_id
                }', '${place.name}', '${place.rating || "N/A"}', '${
        place.vicinity || "Address not available"
      }', '${place.types ? place.types[0] : "place"}')">
              ...
              <div class="col-md-8">
                <div class="place-info">
                  ...
                  <div class="mb-3">
                    ...
                    <span class="badge bg-primary">${type.replace(
                      /_/g,
                      " "
                    )}</span>
                  </div>
